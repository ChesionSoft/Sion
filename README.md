...
空会话提供“梳理本节已有信息”“列出待确认问题”“基于参考资料补充细节”“检查本节遗漏并提出改进建议”四个快捷建议；点击只会填入输入框，仍可编辑并由用户按回车发送。运行记录和每轮状态均可点击打开居中的“运行详情”，查看时间、模型、上下文分解、用量、活动时间线和交付结果；旧版本记录缺少的字段会局部显示“历史记录未保存此信息”，其余可用内容仍正常展示。若服务商不支持所选推理强度，会返回可读的失败信息并提示改为“关闭”后重试。
