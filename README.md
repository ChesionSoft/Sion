...
| **可审阅的 Agent 提案** | 节点对话使用原生工具调用，在对话中读取受限资料并生成可审阅的文稿或规则提案；仅在用户批准后才写入。 |
...
4. 与当前节点的 Agent 对话；它可读取当前文稿、项目附件和直接依赖节点，并提出可逐项预览、批准或拒绝的修改建议。
...
| **原生工具调用** | 节点 Agent 对话必须启用；仅做文本生成的导出流程不要求该能力 |
...
> Sion 桌面运行时不包含浏览器搜索、浏览器自动化或网页抓取子系统。节点 Agent 只可在当前项目内读取当前文稿、全部已索引附件和直接依赖节点，不能访问任意路径、网络、命令或其他项目。

## 附件与 Agent 交付

**附件**：导入的文件复制进项目 `files/` 目录，与提取文本一并管理。支持 TXT / Markdown / JSON / CSV / PDF / DOCX / XLSX；提取失败会明确标记。右侧资料面板可预览提取文本（纯文本，不渲染网页）。节点 Agent 可按受限工具读取当前项目的已索引附件；附件始终只读。勾选附件只提高当前消息的参考优先级，不会扩大权限。文件全文只在本机经 Tauri 读取，不进入前端。

**交付**：Agent 可先回答、读取资料或提出交付稿的变更。一次确认只授权计划中明确列出的节点；计划可按顺序包含多个节点，Rust 会冻结每个节点的 revision，并在每个节点保存前独立执行 Markdown 结构校验、语义审阅和 CAS。语义审阅只返回有限的通过/修正意见，不限制正常 Markdown 表达，也不能直接写文稿。任一节点冲突、审阅重试耗尽或取消都会停止后续节点，已经成功保存的节点保留并逐节点记录审计，不强制回滚。修改当前节点的项目 Agent 规则同样是独立提案，只影响该节点之后的对话。每一次工具读取和校验均在内存中完成，不产生逐工具磁盘写入。
