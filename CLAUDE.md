...
2. **Delivery block parsing** (`delivery-block.ts`) - the assistant ends its reply with a ` ```delivery ` JSON block listing `{sectionKey, patchKind, markdown}` changes. The route parses this into `NodeMarkdownPatch[]` and emits `markdown_patch_preview` events. There is no second model call for document updates.
3. **Markdown patching** (client + `/nodes/[nodeId]/patch`) — patches are applied to the node JSON's `markdown` field through `node-markdown-patcher.ts`; `UnpatchableError` means the patch was rejected. It does not write a `content.md` file. Explicit rewrite remains on `/nodes/[nodeId]/rewrite`.
