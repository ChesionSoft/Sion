...
import { NODES, statusLabel, type AgentRun, type AssistantDeliveryPreview, type ChatMessage, type ChatSession, type NodeId, type ProjectFile, type RecentProject, type WorkflowNode } from "../types";

// Task 5: the existing three-region workbench (node rail + editor + run pane)
// moved wholesale from App, with the file pool delegated to FilePreviewPane.
// Task 7 recomposes this around central chat/draft tabs and a right preview.
type WorkbenchProps = {
...
export function Workbench(props: WorkbenchProps) {
  const {
    ...
    onExit, onSave, onExportDocx, onSelectNode,
  ...
  return (
    <main className="desk-shell workbench-shell">
      <header className="workbench-bar"><button className="wordmark" onClick={onExit} type="button">SION<span>DESKTOP</span></button><div className="project-heading"><span>项目 / {project.name}</span><strong>{nodeTitle}</strong></div><div className="save-state"><span className={dirty ? "dirty-dot" : "clean-dot"} />{dirty ? "有未保存修改" : "已同步本地磁盘"}<button className="export-button" disabled={exporting} onClick={onExportDocx} type="button">{exporting ? "导出中" : "DOCX"}</button><button className="save-button" disabled={!dirty || saving} onClick={onSave} type="button">{saving ? "保存中" : "保存"} <b>⌘/Ctrl S</b></button></div></header>
      <div className="workbench-grid">
        <aside className="node-rail"><div className="rail-title"><span>设计路径</span><b>12</b></div>{NODES.map(([id, title], index) => <button className={id === nodeId ? "node-item selected" : "node-item"} key={id} onClick={() => onSelectNode(id)} type="button"><span>{String(index + 1).padStart(2, "0")}</span><strong>{title}</strong><i>{id === nodeId ? "●" : ""}</i></button>)}<FilePreviewPane files={files} selectedFileIds={selectedFileIds} importing={importingFile} onImport={onImport} onToggleContext={onToggleFile} /></aside>
