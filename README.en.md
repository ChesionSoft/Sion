...
**Deliveries**: An Agent may answer directly, inspect permitted document context, or propose a delivery change. One confirmation authorizes only the nodes explicitly listed in the plan; a plan may contain several nodes in order. Rust freezes each base revision and independently applies Markdown structural validation, bounded semantic review, and CAS before saving each node. Semantic review returns only finite pass/revise feedback and does not restrict normal Markdown expression or write documents. A conflict, exhausted review retry budget, or cancellation stops later nodes; already-saved nodes remain and are audited individually, with no forced rollback. A current-node project Agent-rule change follows the same separate proposal flow and affects only future conversations for that node. Tool reads and validation stay in memory, with no per-tool disk writes.
