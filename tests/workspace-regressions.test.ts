...
test("turn chrome is a thinking-and-execution timeline with a delivery result card", async () => {
  ...
  assert.match(timeline, /ConversationActivitySpinner/);
  assert.match(timeline, /currentActivityId/);
  ...
  assert.match(css, /@keyframes conversation-c2-pulse/);
...
test("AIcss conversation surface keeps delivery-detail access and reduces motion", async () => {
  ...
  assert.match(timeline, /<TimelineMarker status=\{turn\.status\}/);
  assert.match(css, /\.conversation-activity-marker path\s*\{/);
