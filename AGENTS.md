...
Sion is a local-first Tauri 2 desktop application: React/Vite provides the workbench UI and Rust owns all project state, migrations, model networking, file extraction, and export.
...
- `src-tauri/` — versioned Tauri command layer, native dialogs, migration orchestration, provider settings, and DOCX export.
...
- `fixtures/` — legacy layouts used only to test migration.
...
- The desktop runtime has no browser search, browser automation, Playwright, or web egress subsystem. Historical browser metadata may be read only during migration and must not enable new behavior.
