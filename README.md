...
当前公开发布为**未签名开发验证包**（尚未做 Apple 公证 / Windows 代码签名）。从浏览器下载后，macOS 会标记隔离属性，Gatekeeper 可能拦截首次打开，提示“无法验证开发者”“已损坏，无法打开”或安装失败——这通常不是安装包坏了，而是系统拒绝未公证应用。
...
下载 NSIS 安装器（`.exe`）并运行。若 SmartScreen 提示“Windows 已保护你的电脑”，点 **更多信息** → **仍要运行**。

## 快速开始

从源码开发 Sion 需要 Node.js、Rust stable 和当前平台的 Tauri 系统依赖。macOS 上可直接运行；Windows 安装器应在 Windows 上构建。
