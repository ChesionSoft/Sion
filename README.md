...
| **上下文估算** | 输入框旁的圆形指示器估算当前消息相对模型上下文窗口的占比；低于 80% 可用，80%–100% 警告，超过 100% 阻止发送。Sion 不配置最大输出长度，也不自动截断。 |
...
每个会话独立保存所选模型与推理强度（关闭、低、中、高，新会话默认“中”）。在工作区对话输入框旁可随时切换模型或推理强度，选择会写入当前会话；切换会话或重启应用后恢复。输入框旁的圆形指示器估算当前消息（含已选附件）相对模型上下文窗口的占比：低于 80% 为可用，80%–100% 为警告，超过 100% 阻止发送。估算与实际运行使用同一份拼装提示，Sion 不配置最大输出长度、不自动截断提示内容。若服务商不支持所选推理强度，会返回可读的失败信息并提示改为“关闭”后重试。

> Sion 桌面运行时没有浏览器搜索、浏览器自动化、Playwright 或网页抓取子系统。Agent 只基于当前节点、已选择附件和会话工作。

## 附件与 Agent 交付

导入的文件会复制进项目的 `files/` 目录，原件与提取文本一起管理。支持的可提取格式包括 TXT、Markdown、JSON、CSV、PDF、DOCX 和 XLSX；提取失败会明确标记，不会伪装为可用文本。工作台右侧的资料面板可预览已导入文件提取出的文本（仅文本，不渲染网页、不打开外部链接），预览与是否勾选为 Agent 上下文相互独立。在对话输入框勾选的文件只应用于下一条用户消息；发送成功后自动清除勾选，校验失败则保留以便重试。文件全文始终只在本机经 Tauri 读取，不进入前端。
