...
- **工具**：为支持函数工具调用的模型勾选。未勾选时 Sion 使用规划器兜底；它不决定是否可开启联网搜索。
- **默认模型**：设置后作为首选模型。
...
系统填充路径时，Chat Completions 使用 `/v1/chat/completions`，OpenAI Responses 使用 `/v1/responses`；“完整 API URL”模式不会改写你填写的地址。

主页还提供“浏览器搜索”设置。搜索通过本地安全代理发出；可以选择 Google 或百度、系统 Chrome/Edge 或托管 Chromium，并不会使用第三方搜索 API Key。托管 Chromium 不会自动下载，需要在设置中主动安装。

浏览器搜索配置不会自动为聊天开启联网：在每个聊天会话的输入框点击地球图标启用“联网搜索”，该开关按会话保存。用户消息中包含的 HTTP(S) 链接会自动尝试读取。
