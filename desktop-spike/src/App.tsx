  ...
  async function applyAssistant(messageId: string) {
    ...
    try {
      ...
      if (response.conflict) {
        ...
        setNotice(`已将确认的 Assistant 回复写入节点 revision ${response.saved.revision}`);
      }
    } catch (error) {
      setNotice(`应用 Assistant 回复失败：${String(error)}`);
    }
  }
