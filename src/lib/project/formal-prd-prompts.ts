...
export function buildDraftSystemPrompt(): string {
  return [
    ...
    "表格只用于重复可比较的信息；流程使用 ```flow 代码块，内容仅为一条主链路（2–6 个节点，用 -> 或 → 分隔）。",
    "不适用的小节直接省略，不要留空、不要编造事实。",
    "结构硬约束：",
    "- 首行可用一级标题 `# 项目名 产品需求文档（PRD）`，随后用二级标题 `##` 作为章（与蓝图章节标题一致，供 Word 目录收录）。",
    "- 章内用三级标题 `###` 分节；不要跳过层级，不要用四级及以下标题承载主结构。",
    "- 每一 `##` 章下至少有一段可交付正文或表格/列表；禁止只写标题无内容。",
    "- 章节顺序严格跟随蓝图；不要额外插入「目录」「修订记录」等由排版层生成的章节。",
